...
The SemVer contract (flags, exit codes, JSON) is [COMPATIBILITY.md](COMPATIBILITY.md). It binds at the `v1.0.0` tag.

The JSON report fields `scan_id`, `status`, `directory`, `plan_mode`, `total_resources_checked`, `resources_checked_exact`, `total_changed_resources`, `resource_changes`, `started_at`, and `completed_at` are stable for automation. `status` values: `no_drift`, `drift_detected`, `no_changes`, `changes_detected`, `failed`, `skipped` (`skipped` means the scan did not run because `--skip-if-open-pr` found an open PR whose files sit under that Terraform root relative to `--workspace-root` or the current directory). `root_id` is an optional stable opaque root identity, intended for history correlation when `directory` is path-redacted. New optional fields may be added. Consumers must ignore unknown fields and should not depend on text, dashboard, notification, SARIF, JUnit, or Prometheus wording as a stable API. Attribute `before`/`after` may be omitted (`omitempty`) in paths-only reports.
