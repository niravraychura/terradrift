...
- Official Action: `uses: niravraychura/terradrift@vX.Y.Z` after a tag that includes `action.yml`. Empty `version:` then installs that tag. A SHA or branch pin (`uses: …@<sha>`) is not a `v*` tag — always set `with.version: vX.Y.Z` to the matching GitHub Release. `TERRADRIFT_VERSION` / `install.sh` also require a `vX.Y.Z` tag, not a SHA.
