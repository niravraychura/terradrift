...
Pin `uses: niravraychura/terradrift@v1.1.1`. Empty `version:` installs that tag. A SHA or branch pin cannot infer the release — set `version: v1.1.1` as well. `action.yml` has shipped since v0.4.0. `TERRADRIFT_VERSION` for `install.sh` is the same: a `vX.Y.Z` tag, not a SHA.
