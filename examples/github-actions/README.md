...
- **GitHub comments/issues:** add `pull-requests: write` for `--github-pr`, `issues: write` for `--github-issue-after` (one issue per root+fingerprint; closed when that root scans clean). Optional `--github-issue-label` (at most 8).
