...
- `ci.json`: redacted CI output with history throttling, policy, cost, audit adapters, and an external-command allowlist. Always set both `allowed_commands` and `trusted_command_dirs` in CI; GitHub Actions rejects adapters if either list is empty.
