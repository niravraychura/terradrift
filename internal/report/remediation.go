...
// RemediationForActions returns human-reviewed guidance for Terraform changes.
func RemediationForActions(actions []string) string {
	if hasAction(actions, "delete") && hasAction(actions, "create") {
		return "Review the replacement. Confirm whether to update Terraform configuration, import/sync state, or recreate only after approval."
	}
	if hasAction(actions, "delete") {
		return "Review the deletion. Confirm whether to restore the resource, remove it from Terraform state/configuration, or accept the deletion after approval."
	}
	if hasAction(actions, "create") {
		return "Review the unmanaged or missing resource. Confirm whether to import the resource, add Terraform configuration, or remove the unexpected resource after approval."
	}
	if hasAction(actions, "update") {
		return "Review the update. Confirm whether to codify the changed settings in Terraform or revert the infrastructure change after approval."
	}
	...
	return "Review the changed resource with the owning team before changing infrastructure or Terraform state."
}
