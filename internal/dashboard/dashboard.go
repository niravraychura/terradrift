...
<body>
  <main>
    ...
    <table>
      <thead><tr><th>Address</th><th>Type</th><th>Name</th><th>Actions</th><th>Cost impact</th><th>Remediation</th></tr></thead>
      <tbody>{{range .Current.ResourceChanges}}<tr><td>{{.Address}}</td><td>{{.Type}}</td><td>{{.Name}}</td><td>{{range .Actions}}{{.}} {{end}}</td><td>{{.CostImpact}}</td><td>{{.Remediation}}</td></tr>{{else}}<tr><td colspan="6">No changed resources</td></tr>{{end}}</tbody>
    </table>
