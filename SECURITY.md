...
Terraform-backed scans use a local `.terradrift-scan.lock` file by default (`--lock-backend local`) to prevent overlapping scans of the same root on a single host. Shared filesystems can share the lock file across runners on that volume; Redis/Postgres distributed backends are out of scope. After a crash, remove a stale lock manually only when no scan process is active. When `--workspace-root` is set, the resolved directory is re-validated after lock acquisition to reduce symlink TOCTOU risk before Terraform execution. Optional `--webhook-ca-cert` loads a PEM CA bundle for webhook TLS verification in enterprise environments. GitHub issue/PR delivery uses the same SSRF-hardened HTTPS client posture as webhooks (no proxy, no redirects, blocked private destinations, explicit timeout).
